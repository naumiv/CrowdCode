...
//						req.getRequestDispatcher("/404.jsp").forward(req, resp);
				...
				// PROJECT URLS match /word/ or /word/(word)*
					...
					if ( pathSeg.length <= 2 ){
						...
						req.getRequestDispatcher("/client/client.jsp").forward(req, resp); // /clientDist/client.jsp
...
//							req.getRequestDispatcher("/404.jsp").forward(req, resp);
						...
						req.getRequestDispatcher("/statistics/chat.html").forward(req, resp);// /statistics/index.jsp
			...
			final String projectID, User user, final String[] pathSeg) throws IOException, FileUploadException
	{
		...
		} else if (pathSeg[3].equals("fetchAll")) { // new function
			doFetchAll(req, resp, user);
		}
	}
	...
	// new function
	private void doFetchAll (final HttpServletRequest req, final HttpServletResponse resp,final User user) throws IOException{
		// Since the transaction may fail and retry,
		// anything that mutates the values of req and resp MUST be outside the transaction so it only occurs once.
		// And anything inside the transaction MUST not mutate the values produced.
		final String projectID = (String) req.getAttribute("project");
		...
		try {
			jsonResponse=ofy().transact( new Work<String>(){
				...
				public String run() {
					ThreadContext threadContext = ThreadContext.get();
					threadContext.reset();
					return FirebaseService.readDataAbsolute("https://crowdcode2.firebaseio.com/projects/" + projectID + ".json");
			...
			final String projectID, final String[] pathSeg) throws IOException, ServletException
	{
		...
		if(pathSeg.length <=3 ){
			req.getRequestDispatcher("/admin/admin.jsp").forward(req, resp); // /adminDist/admin.jsp
		} else {
